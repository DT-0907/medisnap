...
let ASRLiveTranscriber = (() => {
    ...
    var ASRLiveTranscriber = _classThis = class extends _classSuper {
        ...
        startASR() {
            ...
            options.onTranscriptionUpdateEvent.add((eventArgs) => this.onUpdate(eventArgs));
            options.onTranscriptionErrorEvent.add((error) => this.onError(error));
        ...
        onError(errorCode) {
            ...
            if (this.debugText)
                this.debugText.text = "⚠️ ASR error: " + errorCode;
            this.isActive = false;
        }
        updateDisplay(text) {
            if (this.debugText)
