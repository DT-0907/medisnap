...
export class SupabaseUploader extends BaseScriptComponent {
    ...
    private simulateUpload(uploadUrl: string, base64Data: string, filename: string) {
        ...
        this.uploadToSupabaseStorage(uploadUrl, base64Data, filename);
    }

    private async uploadToSupabaseStorage(uploadUrl: string, base64Data: string, filename: string) {
        try {
            this.updateDebugText("Making HTTP request...");

            // Use Lens Studio's HTTP capabilities
            const headers = {
            ...
            const response = await this.makeHttpRequest(uploadUrl, 'PUT', binaryData, headers);
    ...
    private async makeHttpRequest(url: string, method: string, data: ArrayBuffer, headers: any): Promise<{ success: boolean, error?: string }> {
        try {
            ...
            // Use Lens Studio's RemoteServiceModule for real HTTP requests
            const remoteService = this.sceneObject.createComponent("RemoteServiceModule");

            if (isNull(remoteService)) {
                this.updateDebugText("❌ Cannot create HTTP service");
                return { success: false, error: "HTTP service unavailable" };
            }
            ...
            remoteService.method = method;
            remoteService.headers = headers;
            remoteService.body = base64Data;
