...
let SupabaseUploader = (() => {
    ...
    var SupabaseUploader = _classThis = class extends _classSuper {
        ...
        async uploadRecordedAudio() {
            ...
            if (this.microphoneRecorder.isCurrentlyRecording && this.microphoneRecorder.isCurrentlyRecording()) {
                this.updateDebugText("Please stop recording first!");
                return;
            }
        ...
        prepareUpload() {
            try {
                const wavData = this.tempWavData;
                if (!wavData) {
        ...
        async uploadToSupabaseStorage(filename, wavData) {
            try {
        ...
        finishUpload() {
            try {
                ...
                // Clean up temp data
                this.tempAudioData = null;
                this.tempWavData = null;
        ...
        getRecordedAudioData() {
            // Check if microphone recorder exists and has recorded data
            if (isNull(this.microphoneRecorder)) {
                this.updateDebugText("No microphone recorder found!");
                return new Float32Array(0);
            }
            // Check if there's recorded audio duration
            if (this.microphoneRecorder.recordingDuration <= 0) {
                this.updateDebugText("No audio recorded yet!");
                return new Float32Array(0);
            }
            // Try to access the recorded audio frames safely
            try {
                const recordedFrames = this.microphoneRecorder.recordedAudioFrames;
                if (!recordedFrames || recordedFrames.length === 0) {
                    this.updateDebugText("No recorded frames found!");
                    return new Float32Array(0);
                }
                // Combine all recorded frames into a single array
                const totalSamples = recordedFrames.reduce((sum, frame) => {
                    return sum + (frame.audioFrame ? frame.audioFrame.length : 0);
                }, 0);
                if (totalSamples === 0) {
                    this.updateDebugText("No audio samples found!");
                    return new Float32Array(0);
                }
                const audioBuffer = new Float32Array(totalSamples);
                let offset = 0;
                for (const frame of recordedFrames) {
                    if (frame.audioFrame && frame.audioFrame.length > 0) {
                ...
                this.updateDebugText(`Error accessing audio data: ${error.message}`);
                return new Float32Array(0);
            }
        }
