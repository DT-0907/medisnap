...
export function PatientDetails({ patient }: PatientDetailsProps) {
  ...
  return (
    <div className="w-full space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      {/* Header Card */}
      <Card className="bg-white/30 backdrop-blur-sm p-6 border border-gray-200 rounded-xl shadow-sm">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-4">
            <div className="flex h-16 w-16 items-center justify-center rounded-full bg-yellow-500/30 text-yellow-600 font-bold text-2xl ar-glow">
              {patient.name
                .split(" ")
                .map((n) => n[0])
                .join("")}
        ...
        {showAISummary && (
          <div className="mt-6 p-4 rounded-lg bg-yellow-500/10 border border-yellow-500/30 animate-in fade-in slide-in-from-top-2 duration-300">
        ...
        <div className="flex flex-wrap gap-2">
          {patient.currentSymptoms.map((symptom, index) => (
            <Badge
              key={index}
              variant="secondary"
              className="bg-yellow-500/10 text-card-foreground border-yellow-500/20 px-3 py-1"
            >
              {symptom}
            </Badge>
          ))}
        ...
        <Card className="bg-white/30 backdrop-blur-sm p-6 border border-gray-200 rounded-xl shadow-sm">
          ...
          <div className="space-y-2">
            {patient.allergies.map((allergy, index) => (
              <div key={index} className="flex items-center gap-2 p-2 rounded bg-yellow-500/5">
      ...
      <Card className="bg-white/30 backdrop-blur-sm p-6 border border-gray-200 rounded-xl shadow-sm">
        ...
        <div className="space-y-2">
          {patient.diagnosisHistory.map((diagnosis, index) => (
            <div key={index} className="flex items-center gap-3 p-3 rounded-lg bg-yellow-500/5 border border-yellow-500/20">
              <div className="flex h-8 w-8 items-center justify-center rounded-full bg-yellow-500/20 text-yellow-600 font-bold text-sm">
                {index + 1}
              </div>
